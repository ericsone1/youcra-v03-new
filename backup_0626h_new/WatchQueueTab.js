...
const WatchQueueTab = ({ filter, onFilterChange, handleImageError, handleWatchVideo }) => {
  ...
  useEffect(() => {
    ...
    async function fetchVideos() {
      ...
      const watchedKey = `watchedVideos_${user.uid}`;
      const watchedIds = JSON.parse(localStorage.getItem(watchedKey) || '[]');
      for (const roomDoc of roomsSnapshot.docs) {
        ...
        videosSnapshot.forEach(videoDoc => {
          ...
          if (
            ...
            if (videoData.registeredBy) uploaderIdsSet.add(videoData.registeredBy);
            allVideos.push({
              ...videoData,
              id: videoDoc.id,
              videoId, // 반드시 포함
      ...
      // uploader exposureCredit fetch
      const uploaderCredits = {};
      await Promise.all(Array.from(uploaderIdsSet).map(async(uid)=>{
        try {
          const userDoc=await getDoc(doc(db,'users',uid));
          if(userDoc.exists()){
            uploaderCredits[uid]=userDoc.data().exposureCredit||0;
          }
        }catch(e){console.error('exposure fetch',e);} }));

      const filteredByCredit=allVideos.filter(v=>{
        const credit=uploaderCredits[v.registeredBy];
        return credit===undefined||credit>0;
      });

      console.log('시청 리스트', filteredByCredit);
      setVideos(filteredByCredit);
      setLoading(false);
    }
    fetchVideos();
  }, [user]);
  ...
  // 시청 완료한 영상 id 목록
  const watchedKey = user ? `watchedCounts_${user.uid}` : '';
  const rawWatchedCounts = user ? JSON.parse(localStorage.getItem(watchedKey) || '{}') : {};
  
  // 비정상적으로 큰 숫자들을 정상화 (100 이상이면 1로 리셋)
  const watchedCounts = {};
  let needsUpdate = false;
  Object.keys(rawWatchedCounts).forEach(videoId => {
    const count = rawWatchedCounts[videoId];
    if (count > 100) {
      watchedCounts[videoId] = 1;
      needsUpdate = true;
    } else {
      watchedCounts[videoId] = count;
    }
  });
  
  // 정상화된 데이터를 다시 저장
  if (needsUpdate && user) {
    localStorage.setItem(watchedKey, JSON.stringify(watchedCounts));
  }
          ...
          >
            ...
            <div className="flex gap-2 mt-3">
              <button
                ...
                className={`flex-1 text-white py-2 rounded-lg text-sm font-medium transition-colors ${watchedCounts[video.videoId] ? 'bg-blue-500 hover:bg-blue-600' : 'bg-red-500 hover:bg-red-600'}`}
              >
                {watchedCounts[video.videoId] ? (
                  `${watchedCounts[video.videoId]}번 시청완료`
                ) : (
